...
<style>
    ...
    /* --- 전체 앱 레이아웃 & 배경 --- */
    .stApp {
        background-image: linear-gradient(to bottom right, #f8fafc, #eef2ff); /* 그라데이션 배경 적용 */
    }
    ...
    /* --- 버튼 스타일 (사용자 지정 색상 적용) --- */
    .stButton>button {
        background-color: #5d7ad9; /* 사용자 요청 색상 */
        color: #ffffff !important; /* 흰색 텍스트로 변경 */
        border: 1px solid #4b64c0; /* 어두운 테두리 추가 */
        border-radius: 8px;
    ...
    .stButton>button:hover {
        ...
        background-color: #7a92e3; /* 호버 시 살짝 밝은 색 */
        box-shadow: 0 6px 20px rgba(0, 0, 0, 0.12);
    }
    .stButton>button:active {
        transform: translateY(0);
        background-color: #4b64c0; /* 클릭 시 살짝 어두운 색 */
        box-shadow: 0 4px 15px rgba(0, 0, 0, 0.1);
    }
...
def dashboard_page():
    ...
    df_results['참여도 기여도'] = df_results['engagement_score_normalized'] * st.session_state.w_engagement
    df_results['감성 기여도'] = df_results['sentiment_score_normalized'] * st.session_state.w_sentiment
    
    df_melted = df_results.melt(
        id_vars=['candidate', 'interest_index'],
        value_vars=['참여도 기여도', '감성 기여도'],
        var_name='점수 종류',
        value_name='기여도'
    )
