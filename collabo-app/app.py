...
<style>
    ...
    .report-title-card h2 { color: #1e293b; border: none; margin:0; }
...
@st.cache_data
def load_all_data():
    # 모든 후보의 원본 및 감성 분석 데이터를 로드하고 병합
    all_data = {}
...
def analyze_qualitative_feedback(_candidate_name, tweets_tuple):
    # LLM을 사용하여 정성적인 피드백(토픽, 리스크)을 분석
...
# 캐시 문제를 해결하기 위해 @st.cache_data 데코레이터를 제거
# 함수는 호출될 때마다 항상 실행되어 최신 코멘트를 생성
def generate_marketer_comment(_final_data_str):
    # LLM을 사용하여 마케터의 전략적 코멘트를 생성
...
def detail_page():
    ...
    # 상세 데이터가 없는 경우, 에러 메시지를 표시하고 나머지 UI는 표시하지 않음
    if not detail_data:
        st.error("상세 데이터를 불러올 수 없습니다.")
    else:
        # 상세 데이터가 있는 경우에만 리포트 내용을 표시
